...
func CountBooks() (int64, error) {
	...
	total, err := uleads_engine.Table("Leads_LeadsInfo").Alias("l").
		...
		Where(`l.CreateTime > '2018-01-01' and l.AimUcarId >0
		and l.Category in (1010,1011,1012,1014,2014,5040,5010,5020,5030,3010,5080,5060,8010,8020,8030,8040)`).Count(&u)
	if err != nil {
		return 0, err
	}
	return total, nil
...
func GetBooks(start, end int64) ([]model.Book, error) {
	...
	all := `select ROW_NUMBER() over(order by l.CreateTime desc) as row,
		...
		where l.CreateTime > '2018-01-01' and l.AimUcarId >0
		and l.Category in (1010,1011,1012,1014,2014,5040,5010,5020,5030,3010,5080,5060,8010,8020,8030,8040) `
